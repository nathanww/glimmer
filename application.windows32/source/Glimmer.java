...
import lc.kra.system.mouse.event.*; 
import lc.kra.system.*; 
import lc.kra.system.keyboard.*; 
import lc.kra.system.mouse.*; 
...
public class Glimmer extends PApplet {
...
public void processData(float[] sample, float[] history) {
  ...
  int si=0;
  for (float freq=0.1f; freq <=100; freq=freq+0.1f) { //each bin in spectrum increase by 0.1 Hz.
...
public void runScript(float[] baseline, float[] sample) { //run a user neuroffedback script, called each time we get a new batch of data (frequency depends on the sample length set in the scrpt)
...
public void draw() {
  ...
  if (opMode == 1) { //settings screen, takes over the display with full opacity.
      ...
      rect(100+(frequency*6.66666666667f),190,10,70);
...
public void handleSettings() { //handles mouse interactions with GUI elements on settings screen.
  if (mouseX>=100 && mouseX <=300) { //x ccordinates for sliders
      if (mouseY >=190 && mouseY <= 190+70) { //frequency
        frequency=round((mouseX-100)/6.66666666667f);
      }
...
if (hasScript) {
 ...
 if (mouseX >= 480 && mouseX <=480+300 && mouseY >=200 && mouseY <=300) { //load/stop was clicked
  ...
  public void fileSelected(File selection) { //called when user selects a scriipt file from file chooser
  if (selection == null) {
  ...
  for (int line=0; line < result.length; line++) {
    ...
    if (result[line].indexOf("sampleLength=") > -1) {
      ...
      sampleLength=PApplet.parseInt(result[line].substring(result[line].indexOf("sampleLength=")+13));
    }
      ...
      baselineLength=PApplet.parseInt(result[line].substring(result[line].indexOf("baselineLength=")+15));
    }
...
public void resetFrame() { //resttore flahsing when the mouse is no longer being held. Started after mouse handler detects a button down and pauses flashing
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Glimmer" };
    if (passedArgs != null) {
